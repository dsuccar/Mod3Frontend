...
function fetchUsers(){
  ...
  .then(resp => resp.json())
  .then(users => fetchQuestions(users))
  // .then(users => loginPage(users))
}

function fetchQuestions(users) {
  console.log("Loading questions")
  fetch(QUESTIONS_URL)
    .then(resp => resp.json())
    .then(question => loginPage(question, users))
    // .then(question => renderQuestions(question, user))
}

function loginPage(question, users){
  ...
  loginBtn.addEventListener("click", (event) => submitLogin(event, question, users) )
}
...
function submitLogin(event, question, users){
  ...
  // }else if (input.value !== null && mappedUsers.includes(input.value)){  
    ...
    })//.then(resp => resp.json()).then(user => beforeRenderQuestions(question, user)) did nothing???
    // .then(resp => resp.json()).then(user => fetchQuestions(user))
    // }).then(resp => resp.json()).then(user => renderMain(user))
  }
  beforeRenderQuestions(question, user)
}
